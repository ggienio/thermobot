...
static esp_err_t index_handler(httpd_req_t *req) {
  ...
  if (s != NULL) {
    // if (s->id.PID == OV3660_PID) {
      log_i("Sending index");
      // return httpd_resp_send(req, index_ov3660_html, sizeof(char) * strlen(index_ov3660_html));
      return httpd_resp_send(req, (const char *)index_ov3660_html_gz, index_ov3660_html_gz_len);
    // } else if (s->id.PID == OV5640_PID) {
    //   return httpd_resp_send(req, (const char *)index_ov5640_html_gz, index_ov5640_html_gz_len);
    // } else {
      // return 0;//httpd_resp_send(req, (const char *)index_ov2640_html_gz, index_ov2640_html_gz_len);
    // }
  } else {
...
void startCameraServer() {
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    log_e("Fatal error: web server could not start");
  }
  ...
  if (httpd_start(&stream_httpd, &config) == ESP_OK) {
    ...
    log_e("Fatal error: stream server could not start");
  }
}
